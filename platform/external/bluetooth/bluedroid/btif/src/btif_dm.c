...
#define BTIF_DM_INTERLEAVE_DURATION_BR_TWO    4
...
#if defined(READ_BDNAME_FROM_PROPERTY)
    char prop_bdname[PROPERTY_VALUE_MAX];
#endif
...
#if defined(READ_BDNAME_FROM_PROPERTY)
            property_get((char *)BDNAME_PROPERTY, prop_bdname, (char *)BTM_DEF_LOCAL_NAME);
            strcpy((char *)bd_name->name, (char *)prop_bdname);
            BTIF_TRACE_DEBUG2("%s: Default bluetooth name %s", __FUNCTION__, bd_name->name);
#else

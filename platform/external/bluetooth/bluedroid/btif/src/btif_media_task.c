...
typedef struct
{
    ...
    //BOOLEAN is_edr_supported;//Rhine
#endif
    power_module_t* power_module;
    BOOLEAN raised_cpu_freq;
} tBTIF_MEDIA_CB;
...
static void btif_media_task_raise_cpu_freq(void);
static void btif_media_task_restore_cpu_freq(void);
...
static void btif_media_task_feeding_state_reset(void)
{
    ...
    if (btif_media_cb.TxTranscoding == BTIF_MEDIA_TRSCD_PCM_2_SBC)
    {
        ...
        APPL_TRACE_DEBUG1("bytes_per_tick %u", btif_media_cb.media_feeding_state.pcm.bytes_per_tick);
...
static void btif_media_task_aa_start_tx(void)
{
    ...
    btif_media_task_raise_cpu_freq();
...
static void btif_media_task_aa_stop_tx(void)
{
    ...
    btif_media_task_restore_cpu_freq();
}
...
static UINT8 check_for_max_number_of_frames_per_packet()
{
    ...
    {
        case SBC_sf44100:
            result = (effective_mtu_size - (AVDTP_HDR_SIZE + A2DP_HDR_SIZE)) / MAX_SBC_HQ_FRAME_SIZE_44_1;
            APPL_TRACE_DEBUG1("max number of sbc frames: %d", result);
            break;

        case SBC_sf48000:
            result = (effective_mtu_size - (AVDTP_HDR_SIZE + A2DP_HDR_SIZE)) / MAX_SBC_HQ_FRAME_SIZE_48;
            APPL_TRACE_DEBUG1("max number of sbc frames: %d", result);
            break;
...
static void btif_get_num_aa_frame(UINT8 *numOfIterations, UINT8 *numOfFrames)
{
    UINT8 result=0;
    UINT8 nof = 0;
    UINT8 noi = 1;
    ...
    switch (btif_media_cb.TxTranscoding)
    {
            ...
            */
        {
            ...
            APPL_TRACE_DEBUG1("bytes_per_tick %u", btif_media_cb.media_feeding_state.pcm.bytes_per_tick);
            APPL_TRACE_DEBUG1("pcm.counter %u", btif_media_cb.media_feeding_state.pcm.counter);
...
static power_module_t* btif_media_task_get_powerhal(void)
{
    if (!btif_media_cb.power_module) {
        hw_get_module(POWER_HARDWARE_MODULE_ID,
            (const hw_module_t **)&btif_media_cb.power_module);
    }
    return btif_media_cb.power_module;
}

static void btif_media_task_raise_cpu_freq(void)
{
    if (!btif_media_cb.raised_cpu_freq)
    {
        power_module_t* power = btif_media_task_get_powerhal();
        APPL_TRACE_DEBUG0("Power hint A2DP_ENCODE started");
        if (power && power->powerHint)
            power->powerHint(power, POWER_HINT_A2DP_ENCODE, (void *)"state=1");
        btif_media_cb.raised_cpu_freq = TRUE;
    }
}
...
static void btif_media_task_restore_cpu_freq(void)
{
    ...
    {
        power_module_t* power = btif_media_task_get_powerhal();
        APPL_TRACE_DEBUG0("Power hint A2DP_ENCODE stopped");
        if (power && power->powerHint)
            power->powerHint(power, POWER_HINT_A2DP_ENCODE, (void *)"state=0");
        btif_media_cb.raised_cpu_freq = FALSE;
    }
}
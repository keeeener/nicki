...
/* Link supervision timeout when audio connected in 625us time slots
 * (625us * 8192 = 5.12s)
 */
#define BTIF_HF_SCO_CONNECTED_LINK_TIMEOUT 8192
...
static void btif_hf_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_AG_AUDIO_OPEN_EVT:
            ...
            BTM_SetLinkSuperTout(btif_hf_cb.connected_bda.address,
                BTIF_HF_SCO_CONNECTED_LINK_TIMEOUT);
            break;

        case BTA_AG_AUDIO_CLOSE_EVT:
            HAL_CBACK(bt_hf_callbacks, audio_state_cb, BTHF_AUDIO_STATE_DISCONNECTED, &btif_hf_cb.connected_bda);
            BTM_SetLinkSuperTout(btif_hf_cb.connected_bda.address, HCI_DEFAULT_INACT_TOUT);
            break;
...
/*******************************************************************************
**
** Function         update_caller_id
**
** Description      updates name of caller
**
** Returns          bt_status_t
**
*******************************************************************************/

static bt_status_t update_caller_id(const char *name)
{
    tBTA_AG_RES_DATA ag_res;
    snprintf(ag_res.str, sizeof(ag_res.str) - 1, ",,,\"%s\"", name);

    BTA_AgResult(BTA_AG_HANDLE_ALL, BTA_AG_CALLER_ID_RES, &ag_res);

...
void bta_ag_hfp_result(tBTA_AG_SCB *p_scb, tBTA_AG_API_RESULT *p_result)
{
    ...
    switch(p_result->result)
    {
        ...
        case BTA_AG_CALLER_ID_RES:
            /* make sure that call is processing and clip is set */
            if (p_scb->clip[0] != 0 && p_scb->callsetup_ind == BTA_AG_CALLSETUP_INCOMING)
            {
                strncat(p_scb->clip, p_result->data.str,
                        sizeof(p_scb->clip) - strlen(p_scb->clip) - 1);
                bta_ag_send_ring(p_scb, (tBTA_AG_DATA *) p_result);
            }
            break;

...
int ssp_debug_onoff(char *p_conf_name, char *p_conf_value);
...
int device_class_cfg(char *p_conf_name, char *p_conf_value)
{
    ...
    if(p_token == NULL) {
    	error("pointer p_token for local_device_default_class[0] is NULL!");
    }
    else
    {
    	sscanf(p_token, "%x", &x);
    ...
    if(p_token == NULL) {
    	error("pointer p_token for local_device_default_class[1] is NULL!");
    }
    else
    {
    	sscanf(p_token, "%x", &x);
    ...
    p_token = strtok(NULL, CONF_COD_DELIMITERS);
    if(p_token == NULL) {
    	error("pointer p_token for local_device_default_class[2] is NULL!");
    }
    else
    {
    	sscanf(p_token, "%x", &x);
...
int ssp_debug_onoff(char *p_conf_name, char *p_conf_value)
{
    hci_ssp_debug_enabled = (strcmp(p_conf_value, "true") == 0) ? TRUE : FALSE;
    return 0;
}
/*****************************************************************************

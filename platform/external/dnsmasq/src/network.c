...
#ifdef __ANDROID__
//CONN-EC-WIFI-SS-00957-01+[
void dump_listener(void)
{
    ...
    for (listener = daemon->listeners; listener; listener = listener->next) {
        ...
        my_syslog(LOG_DEBUG, _("daemon->listeners %s, %x"), debug_buff, listener_iface);
    }
}

void adjust_irec_pointer(void)
{
    struct listener *listener;
    struct irec *new_iface;
//    char debug_buff[MAXDNAME];
//    char debug_buff_2[MAXDNAME];
    for (listener = daemon->listeners; listener; listener = listener->next) {
        struct irec *listener_iface = listener->iface;
        for (new_iface = daemon->interfaces; new_iface; new_iface = new_iface->next) {
            if (sockaddr_isequal(&listener_iface->addr, &new_iface->addr)) {
//                prettyprint_addr(&listener_iface->addr, debug_buff);
//                prettyprint_addr(&new_iface->addr, debug_buff_2);
//                my_syslog(LOG_DEBUG, _("%s, %x, %s, %x"), debug_buff, listener_iface, debug_buff_2, new_iface);
...
void set_interfaces(const char *interfaces)
{
    ...
    adjust_irec_pointer(); //CONN-EC-WIFI-SS-00957-01+
